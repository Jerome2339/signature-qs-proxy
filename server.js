...
function rEX(){
  ...
  const stabs='<div class="subtabs">'+['Cost Estimate','Elemental Detail','Planning & Regs','VAT Summary','Fee Calculator','Extend vs Move','Budget vs Actual','Materials'].map((l,i)=>{const onclick='data-ctx='+ctx+' data-i='+i+' onclick="window.mxTabClick(this)"';return'<button class="stab'+(m.tab===i?' active':'')+'" '+onclick+'>'+l+'</button>';}).join('')+'</div>';
  let b='';
  if(exTab===0){
