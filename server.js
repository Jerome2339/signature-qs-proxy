...
function spCalc(){
  var ratio=SP.gia/140;
  var b=0;
  SP.secs.forEach(function(s){
    s.tot=s.its.reduce(function(t,it){
      var q=(it.u==='nr'||it.u==='item'||it.u==='sum')?(+it.q||0):(+it.q||0)*ratio;
      it.tot=q*(+it.r||0);it.scaledQ=Math.round(q*10)/10;
      return t+it.tot;
    },0);
    b+=s.tot;
  });
  var c=b*(SP.con/100),g=b+c;
  return{base:b,pre:0,con:c,grand:g,m2:g/SP.gia};
}
...
function rSP(){
  const t=spCalc();
  const tabs=[['cp','Cost plan'],['bva','Budget vs Actual'],['cv','Client view'],['mx','Materials']];
  let h='<div class="ms"><div class="mc"><div class="ml">Grand total</div><div class="mv go">'+F(t.grand)+'</div></div><div class="mc"><div class="ml">Cost/m\u00b2</div><div class="mv">'+F(t.m2)+'/m\u00b2</div></div><div class="mc"><div class="ml">Contingency</div><div class="mv">'+F(t.con)+'</div></div></div><div style="display:flex;gap:0;border:1px solid var(--bd);border-radius:6px;overflow:hidden;margin-bottom:.9rem">'+tabs.map(([v,l])=>'<button onclick="SP.view=\''+v+'\';rSP()" style="flex:1;padding:7px 4px;font-size:10px;font-weight:500;letter-spacing:.06em;text-transform:uppercase;background:'+(SP.view===v?'var(--g)':'transparent')+';color:'+(SP.view===v?'#111':'var(--mu)')+';border:none;cursor:pointer;font-family:\'DM Sans\',sans-serif;border-right:1px solid var(--bd)">'+l+'</button>').join('')+'</div>';
  if(SP.view==='cp'){
    h+='<div class="ib blue" style="margin-bottom:.75rem"><strong>Getting started:</strong> Enter your Gross Internal Floor Area below in metric (m²) or imperial — the calculator instantly generates a full RIBA-structured cost plan for your project, with elemental rates you can adjust to suit your specification and location.</div>';
    h+='<div class="cd"><div class="ct">Project details</div><div class="g3"><div class="f"><label>Project</label><input value="'+SP.proj+'" onchange="SP.proj=this.value"/></div><div class="f"><label>Client</label><input value="'+SP.client+'" onchange="SP.client=this.value"/></div><div class="f"><label>GIA (m\u00b2)</label><input type="number" value="'+SP.gia+'" onchange="SP.gia=+this.value;rSP()"/></div><div class="f"><label>Stage</label><select onchange="SP.stage=+this.value;rSP()">'+[1,2,3,4,5].map(s=>'<option value="'+s+'"'+(s===SP.stage?' selected':'')+'>Stage '+s+'</option>').join('')+'</select></div><div class="f"><label>Contingency %</label><input type="number" value="'+SP.con+'" onchange="SP.con=+this.value;rSP()"/></div></div></div>';
  ...
  if(SP.view==='bva'){
    ...
    h+='<div class="ib" style="margin-bottom:.75rem">This comparison tool allows you to record your actual costs against your cost plan as your build progresses, highlighting savings and overspends as they happen — so you can adjust your budget and stay in control of your project from start to finish.</div>';
  ...
  if(SP.view==='mx'){h+='<div class="ib blue" style="margin-bottom:.75rem"><strong>The most powerful feature of the calculator.</strong> Simply upload your architect\'s drawings in PDF format — our AI analyses them and produces a comprehensive materials schedule covering everything you need for your project: bricks, blocks and mortar, floor joists, staircases, windows and doors, roof tiles, rainwater goods and everything in between. Export as PDF or send directly to your selected trade partners for pricing — including kitchens, bathrooms, tiling and more.</div>';
    h+=renderMXInline('sp');}
  document.getElementById('v1').innerHTML=wrap('Self Build Home Calculator','RIBA cost plan \u00b7 Budget vs actual \u00b7 Materials schedule',h,1,'<button class="bg" onclick="spCP()">Cost Plan PDF</button><button class="bg" onclick="spBvA()">BvA PDF</button>');
}
...
function rHB(){
  ...
  const subTabs='<div class="subtabs">'+['Cost Plan','Budget vs Actual','Client View','Materials'].map((l,i)=>'<button class="stab'+(hbTab===i?' active':'')+'" onclick="setHBTab('+i+')">'+l+'</button>').join('')+'</div>';
  let body='';
  if(hbTab===0){
  ...
  if(hbTab===3){body+=renderMXInline('hb');}
  document.getElementById('v2').innerHTML=wrap('Housebuilder Platform','ft\u00b2/m\u00b2 rate bands \u00b7 Spec uplifts \u00b7 GDV matrix \u00b7 Land value',metrics+subTabs+body,2,'<button class="bg" onclick="hbCostPDF()">Cost Plan PDF</button><button class="bg" onclick="hbApprPDF()">Appraisal PDF</button>');
}
function rFS(){
  const t=fsCalc();
...
function hbCostPDF(){const hbt=hbTot(),up=hbSpecUplift(),baseFt2=hbBaseFt2();let r=HB.types.filter(t=>t.en&&t.n>0).map(t=>{const c=hbCostUnit(t.gia),rm2=Math.round(hbRateM2());return'<tr><td>'+t.l+'</td><td>'+t.gia+'m\u00b2</td><td>'+t.n+'</td><td>\u00a3'+rm2+'/m\u00b2</td><td>'+F(c)+'</td><td>'+F(c*t.n)+'</td></tr>';}).join('');opdf('<h1>Housebuilder Cost Plan</h1><h2>'+hbt.u+' plots \u00b7 \u00a3'+baseFt2+'/ft\u00b2 base + \u00a3'+up.total+'/m\u00b2 spec uplifts \u00b7 '+TD()+'</h2><table><thead><tr><th>Type</th><th>GIA</th><th>Plots</th><th>Rate/m\u00b2</th><th>Cost/plot</th><th>Total</th></tr></thead><tbody>'+r+'<tr class="tr"><td colspan="5">Total build cost</td><td>'+F(hbt.b)+'</td></tr></tbody></table><p class="note">Prelims '+HB.pre+'% \u00b7 Prelims '+HB.pre+'% \u00b7 Contingency '+HB.con+'% \u00b7 Spec: '+HB.wall+' walls, '+HB.roof+' roof, '+HB.ground+' ground \u00b7 Prepared by Signature Construction Projects Ltd</p>','HB Cost Plan');}
...
function rEX(){
  ...
  const stabs='<div class="subtabs">'+['Cost Estimate','Elemental Detail','Planning & Regs','VAT Summary','Fee Calculator','Extend vs Move','Budget vs Actual','Materials'].map((l,i)=>{return'<button class="stab'+(exTab===i?' active':'')+'" onclick="setEXTab('+i+')">'+l+'</button>';}).join('')+'</div>';
  let b='';
  if(exTab===0){
...
function mxCalc(){
  ...
  if(MX.wallType==='cavity'){
    mats.push({cat:'Masonry',item:'Facing bricks (outer leaf)',qty:Math.ceil(ewn*66*MXW),unit:'nr',note:'60/m2 stretcher bond + 10% waste',basis:ewn.toFixed(1)+'m2 net'});
...
function mkMXState(){
  return {
    ...
    mode:'house',storeys:1,stairs:false,
    walls:[],openings:[],
    ...
    pv:{postcode:'',orientation:'S',roofAreaOverride:0,pitchOverride:0,result:null}
  };
}

const MXS={sp:mkMXState(),hb:mkMXState(),fs:mkMXState(),ex:mkMXState()};
MXS.ex.mode='extension';

function getMX(ctx){return MXS[ctx]||MXS.sp;}

function mxiFA(m){return mxFN(m.floor.L)*mxFN(m.floor.W);}
function mxiEWN(m){
  const g=m.walls.filter(function(w){return w.ext && !(m.mode==='extension' && (w.cls==='party'||w.cls==='knock'));}).reduce(function(t,w){
    // For gable walls use rectangular area + gable triangle if specified
    var rectArea=mxFN(w.L)*mxFN(w.H);
    var gableArea=mxFN(w.gable_area_m2)||0;
    return t+rectArea+gableArea;
  },0);
  const o=m.openings.reduce(function(t,o2){return t+mxFN(o2.W)*mxFN(o2.H)*mxFN(o2.qty);},0);
  return Math.max(0,g-o);
}
function mxiAWA(m){return mxiEWN(m)+m.walls.filter(w=>!w.ext).reduce((t,w)=>t+mxFN(w.L)*mxFN(w.H),0);}
function mxiPerim(m){ if(m.mode==='extension'){return m.walls.filter(function(w){return w.ext&&w.cls!=='party'&&w.cls!=='knock';}).reduce(function(t,w){return t+mxFN(w.L);},0);} return m.walls.reduce(function(t,w){return t+mxFN(w.L);},0);}
...
function mxiCalc(m){
  ...
  if(m.wallType==='cavity'){
    mats.push({cat:'Masonry',item:'Facing bricks (outer leaf)',qty:Math.ceil(ewn*66*MXW),unit:'nr',note:'60/m2 stretcher bond + 10% waste',basis:ewn.toFixed(1)+'m2'});
    mats.push({cat:'Masonry',item:'Mortar brickwork (25kg bags)',qty:Math.ceil(ewn*0.6*MXW),unit:'bags',note:'0.6 bags/m2 facing bricks',basis:ewn.toFixed(1)+'m2'});
    mats.push({cat:'Masonry',item:'Concrete blocks 100mm dense (inner leaf)',qty:Math.ceil(awa*11*MXW),unit:'nr',note:'10 blocks/m2 + 10% waste',basis:awa.toFixed(1)+'m2'});
    mats.push({cat:'Masonry',item:'Mortar blockwork (25kg bags)',qty:Math.ceil(awa*0.35*MXW),unit:'bags',note:'0.35 bags/m2',basis:awa.toFixed(1)+'m2'});
  ...
  // Use extracted ground floor area if available, otherwise use fa
  var faGround=mxFN(m.extractedSpec&&m.extractedSpec.floorAreas?m.extractedSpec.floorAreas.ground_m2:0)||fa;
  var faFirst=mxFN(m.extractedSpec&&m.extractedSpec.floorAreas?m.extractedSpec.floorAreas.first_m2:0)||0;
  var jSpacing=mxFN(m.joistSpacing)/1000||0.4;
  if(m.mode==='extension'){ faGround=fa; faFirst=(mxFN(m.storeys)>=2?fa:0); }

  if(m.floorType==='concrete'){
    mats.push({cat:'Ground Floor (Concrete Slab)',item:'Sharp sand blinding 50mm',qty:mxF2(faGround*0.05*MXW),unit:'m3',note:'Below DPM',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Concrete Slab)',item:'DPM 300mu polythene',qty:mxF2(faGround*MXW),unit:'m2',note:'Lapped and taped',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Concrete Slab)',item:'Floor insulation 100mm PIR',qty:mxF2(faGround*MXW),unit:'m2',note:'Meets Part L U-value 0.13',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Concrete Slab)',item:'Ready-mix concrete C25 100mm slab',qty:mxF2(faGround*0.1*MXW),unit:'m3',note:'Ground bearing, pump rate if needed',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Concrete Slab)',item:'A142 welded steel mesh',qty:mxF2(faGround*MXW),unit:'m2',note:'200x200mm 3.55kg/m2',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Concrete Slab)',item:'Edge insulation 100mm EPS',qty:mxF2(perim*0.25*MXW),unit:'lin m',note:'Perimeter edge insulation',basis:perim.toFixed(1)+'m perimeter'});
  } else {
    // Beam & block ground floor
    var nBeams=Math.ceil(mxFN(m.floor.W)/0.6)+1;
    mats.push({cat:'Ground Floor (Beam & Block)',item:'Precast concrete beams',qty:Math.ceil(nBeams*mxFN(m.floor.L)*MXW),unit:'lin m',note:'At 600mm c/c',basis:nBeams+' beams'});
    mats.push({cat:'Ground Floor (Beam & Block)',item:'Concrete infill blocks 100mm',qty:Math.ceil(faGround*11*MXW),unit:'nr',note:'10 blocks/m2 + 10% waste',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Beam & Block)',item:'Sand/cement grout',qty:mxF2(faGround*0.02*MXW),unit:'m3',note:'Joints between blocks',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Beam & Block)',item:'Floor insulation 100mm PIR',qty:mxF2(faGround*MXW),unit:'m2',note:'Over beam & block, under screed',basis:faGround.toFixed(1)+'m2'});
    mats.push({cat:'Ground Floor (Beam & Block)',item:'Sand/cement screed 65mm',qty:mxF2(faGround*0.065*MXW),unit:'m3',note:'Over insulation, power floated',basis:faGround.toFixed(1)+'m2'});
  }

  // ── FIRST FLOOR CONSTRUCTION (Timber) ────────────────────────────────────
  if(faFirst>0){
    var floorW=mxFN(m.floor.W)||Math.sqrt(faFirst);
    var floorL=faFirst/floorW;
    var nJ=Math.ceil(floorW/jSpacing)+1;
    // Joist size based on span (standard: 47x195 up to 4.5m, 47x220 up to 5.5m, 47x245 up to 6m)
    var joistSize=floorW<=4.5?'47x195mm':floorW<=5.5?'47x220mm':'47x245mm';
    mats.push({cat:'First Floor (Timber)',item:'Floor joists '+joistSize+' C24',qty:Math.ceil(nJ*floorL*MXW),unit:'lin m',note:'At '+m.joistSpacing+'mm c/c — span '+floorW.toFixed(1)+'m',basis:nJ+' joists'});
    mats.push({cat:'First Floor (Timber)',item:'Joist hangers galv steel (BAT or similar)',qty:Math.ceil(nJ*2*MXW),unit:'nr',note:'Both ends of each joist',basis:nJ+' joists x2'});
    mats.push({cat:'First Floor (Timber)',item:'Solid noggins 47x'+joistSize.split('x')[1],qty:Math.ceil(floorL/1.8*nJ*0.47*MXW),unit:'lin m',note:'At 1800mm c/c between joists',basis:'1 row per 1.8m span'});
    mats.push({cat:'First Floor (Timber)',item:'Herringbone strutting (galv steel)',qty:Math.ceil(floorL/1.8*(nJ-1)*MXW),unit:'nr',note:'Lateral restraint at mid-span',basis:'1 per joist bay per row'});
    mats.push({cat:'First Floor (Timber)',item:'T&G chipboard 22mm P5 moisture resistant',qty:mxF2(faFirst*MXW),unit:'m2',note:'Glued and screwed to joists',basis:faFirst.toFixed(1)+'m2'});
    mats.push({cat:'First Floor (Timber)',item:'Acoustic mineral wool quilt 100mm',qty:mxF2(faFirst*MXW),unit:'m2',note:'Between joists for sound insulation Part E',basis:faFirst.toFixed(1)+'m2'});
    mats.push({cat:'First Floor (Timber)',item:'Resilient bars (acoustic)',qty:mxF2(faFirst*MXW/0.6*MXW),unit:'nr',note:'Ceiling side — 600mm c/c for sound',basis:faFirst.toFixed(1)+'m2'});
    mats.push({cat:'First Floor (Timber)',item:'Plasterboard 15mm Soundbloc ceiling',qty:Math.ceil(faFirst*MXW/2.88),unit:'nr',note:'Below joists on resilient bars',basis:faFirst.toFixed(1)+'m2'});
  }
  ...
  if(m.roofType==='flat'){
    ...
    const rl=mxiRL(m);
    // Trussed rafters at 600mm centres (standard for prefabricated roof trusses)
    var trussSpacing=0.6;
    var nTruss=Math.ceil((rln+ov*2)/trussSpacing)+1;
    mats.push({cat:'Roof (Pitched)',item:'Trussed rafters (Fink) to span '+sp.toFixed(1)+'m',qty:Math.ceil(nTruss*MXW),unit:'nr',note:'At 600mm c/c, prefabricated to engineer design, '+m.roof.pitch+'\u00b0 pitch',basis:(rln+ov*2).toFixed(1)+'m roof length'});
    mats.push({cat:'Roof (Pitched)',item:'Truss clips / framing anchors',qty:Math.ceil(nTruss*2*MXW),unit:'nr',note:'2 per truss to wall plate',basis:nTruss+' trusses x2'});
    mats.push({cat:'Roof (Pitched)',item:'Galv truss bracing 22x97mm',qty:Math.ceil((rln+ov*2)*3*MXW),unit:'lin m',note:'Longitudinal + diagonal + chevron bracing',basis:'BS 5268 bracing'});
    mats.push({cat:'Roof (Pitched)',item:'Lateral restraint straps 30x5mm',qty:Math.ceil((rln+ov*2)/2*MXW),unit:'nr',note:'Gable & wall plate restraint at 2m c/c',basis:(rln+ov*2).toFixed(1)+'m'});
  ...
  if(m.openings.length>0){
    ...
    if(wins.length>0){
      ...
      var headGroups={};
      wins.forEach(function(o){var w=mxFN(o.W);if(!w)return;var len=Math.ceil((w+0.3)*1000/50)*50;headGroups[len]=(headGroups[len]||0)+(o.qty||1);});
      Object.keys(headGroups).sort(function(a,b){return a-b;}).forEach(function(len){var qty=headGroups[len];mats.push({cat:'Structural',item:'Cast stone head '+len+'mm',qty:Math.ceil(qty*MXW),unit:'nr',note:'Window width + 300mm bearing (150mm each side)',basis:qty+' window(s)'});});
    }
  ...
  // \u2500\u2500 STAIRCASE (new footprint, when an upper floor is added) \u2500\u2500
  if(m.stairs){
    mats.push({cat:'Staircase',item:'Softwood staircase \u2014 straight flight, 13 riser (allow)',qty:1,unit:'nr',note:'Standard flight; winders/turn to detail',basis:'1 flight'});
    mats.push({cat:'Staircase',item:'Balustrade set \u2014 handrail, spindles & newels',qty:1,unit:'set',note:'To match flight',basis:'1 set'});
    mats.push({cat:'Staircase',item:'Trimmer joists & make good to stairwell (allow)',qty:1,unit:'item',note:'Trimming around new opening',basis:'1 opening'});
  }

  // \u2500\u2500 EXTENSION JUNCTIONS & TIE-INS (only what joins to the existing house) \u2500\u2500
  if(m.mode==='extension'){
    var partyW=m.walls.filter(function(w){return w.cls==='party'||w.cls==='knock';});
    var knockW=m.walls.filter(function(w){return w.cls==='knock';});
    var abutLen=partyW.reduce(function(t,w){return t+mxFN(w.L);},0);
    var abutH=partyW.reduce(function(t,w){return t+mxFN(w.H);},0);
    if(partyW.length>0){
      mats.push({cat:'Junctions & Tie-ins',item:'Wall starter profile stainless (full height)',qty:Math.ceil(partyW.length*MXW),unit:'nr',note:'Fixed to existing wall at each junction',basis:partyW.length+' junction(s)'});
      var tieQty=partyW.reduce(function(t,w){return t+Math.ceil(mxFN(w.H)/0.3);},0);
      mats.push({cat:'Junctions & Tie-ins',item:'Wall starter ties stainless',qty:Math.ceil(tieQty*MXW),unit:'nr',note:'At 300mm vertical centres',basis:partyW.length+' junction(s)'});
      mats.push({cat:'Junctions & Tie-ins',item:'Vertical DPC at abutment',qty:mxF2(abutH*MXW),unit:'lin m',note:'Between new and existing wall',basis:partyW.length+' junction(s)'});
    }
    if(abutLen>0){
      if(m.roofType!=='flat'){
        mats.push({cat:'Junctions & Tie-ins',item:'Lead flashing code 4 to roof abutment',qty:mxF2(abutLen*MXW),unit:'lin m',note:'Where new roof meets existing wall',basis:abutLen.toFixed(1)+'m abutment'});
        mats.push({cat:'Junctions & Tie-ins',item:'Stepped cavity tray / DPC over abutment',qty:mxF2(abutLen*MXW),unit:'lin m',note:'Over abutment line',basis:abutLen.toFixed(1)+'m'});
      } else {
        mats.push({cat:'Junctions & Tie-ins',item:'Cover flashing & upstand to flat roof abutment',qty:mxF2(abutLen*MXW),unit:'lin m',note:'Where new flat roof meets existing wall',basis:abutLen.toFixed(1)+'m'});
      }
    }
    if(knockW.length>0){
      mats.push({cat:'Junctions & Tie-ins',item:'Steel beam to knock-through \u2014 size by SE (allow)',qty:knockW.length,unit:'nr',note:'Structural opening between existing & new; engineer to design',basis:knockW.length+' opening(s)'});
      mats.push({cat:'Junctions & Tie-ins',item:'Padstones / spreaders to beam bearings',qty:Math.ceil(knockW.length*2*MXW),unit:'nr',note:'One each bearing',basis:knockW.length+' beam(s)'});
    }
    mats.push({cat:'Junctions & Tie-ins',item:'Make good & match existing (brick/render/finishes) (allow)',qty:1,unit:'item',note:'Matching materials & junction making-good',basis:'Allowance'});
  }
...
function renderMXInline(ctx){
  ...
  const tabs=['Upload Drawings','Dimensions','Spec & Roof','Openings','Schedule','Kitchen & Bathroom','Solar PV'];
  ...
  if(m.tab===1){
    ...
    body+='<div class="cd"><div class="ct">Take-off mode</div><div class="trow">'+[['house','Whole building'],['extension','Extension only']].map(function(kl){return '<div class="tog'+(m.mode===kl[0]?' on':'')+'" data-ctx='+ctx+' data-mode="'+kl[0]+'" onclick="window.mxSetMode(this)">'+kl[1]+'</div>';}).join('')+'</div>';
    if(m.mode==='extension'){
      body+='<div class="g3" style="margin-top:.6rem"><div class="f"><label>Storeys</label><select data-ctx='+ctx+' onchange="window.mxSetStoreys(this)" style="width:100%;font-size:12px;padding:6px 8px;border:1px solid var(--bd);border-radius:4px;background:rgba(255,255,255,.06);color:var(--w);font-family:\'DM Sans\',sans-serif"><option value="1"'+(m.storeys>=2?'':' selected')+'>Single storey</option><option value="2"'+(m.storeys>=2?' selected':'')+'>Two storey</option></select></div><div class="f"><label>New staircase</label><label style="display:flex;align-items:center;gap:6px;font-size:12px;color:#ccc;padding-top:7px"><input type="checkbox" '+(m.stairs?'checked':'')+' data-ctx='+ctx+' onchange="window.mxToggleStairs(this)" style="accent-color:var(--g);width:14px;height:14px"/> Include</label></div></div>';
      body+='<div class="ib" style="border-left-color:var(--g)">Footprint = Length × Width below. In the Walls list, classify any wall that joins the existing house so it is excluded \u2014 new junctions, any knock-through beam and make-good are added automatically.</div>';
    }
    body+='</div>';
    ...
    if(m.mode==='extension'){
      body+='<div class="cd"><div class="ct">Wall classification (extension)</div><div class="ib">New external = taken off in full. Party / shared = abuts the existing house, excluded. Knock-through = wall with the structural opening into the house.</div>';
      m.walls.forEach(function(w,i){
        body+='<div style="display:grid;grid-template-columns:1fr 210px;gap:8px;align-items:center;margin-bottom:5px"><span style="font-size:11px;color:#ccc">'+w.label+'</span><select data-ctx='+ctx+' data-i='+i+' onchange="window.mxWallClass(this)" style="font-size:11px;padding:5px 7px;border:1px solid var(--bd);border-radius:4px;background:rgba(255,255,255,.06);color:var(--w);font-family:\'DM Sans\',sans-serif">'+[['new','New external'],['party','Party / shared (exclude)'],['knock','Knock-through']].map(function(o){return '<option value="'+o[0]+'"'+((w.cls||'new')===o[0]?' selected':'')+'>'+o[1]+'</option>';}).join('')+'</select></div>';
      });
      body+='</div>';
    }
  }
  ...
  // ── TAB 6: SOLAR PV ─────────────────────────────────────────────────────
  if(m.tab===6){
    var pv=m.pv||{postcode:'',orientation:'S',roofAreaOverride:0,pitchOverride:0,result:null};
    body+='<div class="cd"><div class="ct">Solar PV Estimator</div>';
    body+='<div class="ib blue">Enter your postcode and roof orientation to estimate the optimum solar PV system size, expected annual generation, battery and inverter sizing, and indicative cost. This is a feasibility estimate for budgeting \u2014 a full design requires an MCS-certified site survey.</div>';

    // Inputs
    body+='<div class="g2" style="margin-top:.75rem">';
    body+='<div class="f"><label>Postcode (first part, e.g. DN1)</label><input value="'+(pv.postcode||'')+'" data-ctx="'+ctx+'" onchange="window.mxPVField(this,\'postcode\')" placeholder="DN1"/></div>';
    body+='<div class="f"><label>Roof orientation (direction it faces)</label><select data-ctx="'+ctx+'" onchange="window.mxPVField(this,\'orientation\')">';
    var orients=[['S','South (best)'],['SE','South-East'],['SW','South-West'],['E','East'],['W','West'],['NE','North-East'],['NW','North-West'],['N','North (poorest)']];
    orients.forEach(function(o){body+='<option value="'+o[0]+'"'+(pv.orientation===o[0]?' selected':'')+'>'+o[1]+'</option>';});
    body+='</select></div>';
    body+='</div>';

    // Roof area and pitch - auto from drawings with override
    var autoArea=mxF2(mxiRA(m));
    var autoPitch=mxFN(m.roof.pitch)||35;
    body+='<div class="g2">';
    body+='<div class="f"><label>Roof area available (m\u00b2)'+(autoArea>0?' \u2014 auto: '+autoArea+'m\u00b2':'')+'</label><input type="number" value="'+(pv.roofAreaOverride||autoArea||0)+'" data-ctx="'+ctx+'" onchange="window.mxPVField(this,\'roofAreaOverride\')" placeholder="'+(autoArea||50)+'"/></div>';
    body+='<div class="f"><label>Roof pitch (\u00b0)'+(autoPitch?' \u2014 auto: '+autoPitch+'\u00b0':'')+'</label><input type="number" value="'+(pv.pitchOverride||autoPitch||35)+'" data-ctx="'+ctx+'" onchange="window.mxPVField(this,\'pitchOverride\')" placeholder="35"/></div>';
    body+='</div>';

    body+='<button class="bg" style="background:var(--bl);color:#fff;width:100%;padding:.85rem;margin-top:.5rem" data-ctx="'+ctx+'" onclick="window.mxPVCalc(this)">Calculate Solar PV System</button>';

    // Results
    if(pv.result){
      var r=pv.result;
      body+='<div class="cd" style="margin-top:.75rem"><div class="ct">Recommended system</div>';
      body+='<div class="ms" style="margin-bottom:.75rem">';
      body+='<div class="mc"><div class="ml">System size</div><div class="mv go">'+r.kwp+' kWp</div></div>';
      body+='<div class="mc"><div class="ml">Panels</div><div class="mv">'+r.panels+'</div></div>';
      body+='<div class="mc"><div class="ml">Annual yield</div><div class="mv">'+r.yield.toLocaleString()+' kWh</div></div>';
      body+='<div class="mc"><div class="ml">Est. annual saving</div><div class="mv go">\u00a3'+r.saving.toLocaleString()+'</div></div>';
      body+='</div>';
      body+='<table class="mtbl2"><thead><tr><th>Component</th><th>Spec</th><th>Qty</th></tr></thead><tbody>';
      body+='<tr><td>Solar panels</td><td>440W monocrystalline</td><td>'+r.panels+'</td></tr>';
      body+='<tr><td>Inverter</td><td>'+r.inverter+' hybrid</td><td>1</td></tr>';
      body+='<tr><td>Battery storage</td><td>'+r.battery+' kWh</td><td>1</td></tr>';
      body+='<tr><td>Mounting & rails</td><td>In-roof or on-roof kit</td><td>'+r.panels+' panel kit</td></tr>';
      body+='<tr><td>DC/AC isolators & cabling</td><td>MCS compliant</td><td>1 set</td></tr>';
      body+='<tr><td>Generation meter</td><td>MID approved</td><td>1</td></tr>';
      body+='</tbody></table>';
      body+='<div class="ib green" style="margin-top:.75rem"><strong>Indicative installed cost: \u00a3'+r.cost.toLocaleString()+'</strong> (inc. panels, battery, inverter, mounting, installation). Payback approx '+r.payback+' years at current energy prices.</div>';
      body+='<div class="ib" style="margin-top:.5rem;font-size:11px">Orientation factor: '+r.orientPct+'% \u00b7 Regional irradiance: '+r.irradiance+' kWh/m\u00b2/yr \u00b7 Pitch factor: '+r.pitchPct+'%. Figures are indicative and assume no significant shading. A full MCS survey is required for installation.</div>';
      body+='</div>';
    }
    body+='</div>';
  }
