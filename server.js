...
Rules: Only extract explicitly annotated dimensions. Convert mm to m. Never estimate from scale. Return null for missing values.`
    ...
    const anthropicRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-sonnet-4-5', max_tokens: 4000, messages: [{ role: 'user', content: contentParts }] }),
    });
    ...
    try {
      ...
      console.error('JSON parse failed, attempting recovery. Raw length:', raw.length);
      try {
        ...
        for (let i = lines.length; i > 5; i--) {
          ...
          if (diff >= 0) {
            attempt += '\n' + '}'.repeat(diff);
            try {
              recovered = JSON.parse(attempt);
              console.log('JSON recovery succeeded at line', i);
              break;
            } catch(e2) {}
          }
        }
        if (recovered) {
          parsed = recovered;
          if (!parsed.missing) parsed.missing = [];
          parsed.missing.push('Response was truncated — some data may be incomplete. Try with a simpler drawing or fewer pages.');
        } else {
          return res.status(502).json({ error: 'Could not parse AI response — try uploading fewer pages at once', raw: raw.substring(0, 500) });
        }
      } catch(e2) {
